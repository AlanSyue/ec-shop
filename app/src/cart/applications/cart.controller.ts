...
import { Controller, Get, UseGuards, Req, Body, Post, Param, Delete } from '@nestjs/common';
...
export class CartController {
    ...
    @UseGuards(JwtAuthGuard)
    @Post(':productId')
    async addToCart(@Req() req, @Param('productId') productId: number, @Body() cartDto: CartDto) {
        try {
            const userId = req.user.id
    ...
    @UseGuards(JwtAuthGuard)
    @Delete(':productId')
    async removeFromCart(@Req() req, @Param('productId') productId: number, @Body() cartDto: CartDto) {
        try {
            const userId = req.user.id
