...
describe('CartController', () => {
  ...
  describe('test find cart method', () => {
    it('should return cart products', async () => {
      ...
      const serviceFindCartSpy = jest.spyOn(mockCartService, 'findCart')
  ...
  describe('test checkout method', () => {
    it('should return order info', async () => {
      ...
      const serviceCheckoutSpy = jest.spyOn(mockCartService, 'checkout')
        .mockImplementation(async () => {
          return orderId
        });

      const orderServiceFindSpy = jest.spyOn(mockOrderService, 'find')
        .mockImplementation(async () => {
          return orderInfo
        });
  ...
  describe('test add to cart method', () => {
    it('should add to cart', async () => {
      ...
      expect(serviceCheckoutSpy).toBeCalledTimes(1)
      expect(serviceCheckoutSpy).toBeCalledWith(request.user.id, productId, cartDto.amount)
    })
  })
